...
# FIX THE R SCRIPT outputPDF path for INDELs

( awk 'NR<=24' $CORE_PATH/$PROJECT/JOINT_CALL/$MDNA_HASH_ADDRESS/VQSR/variants.indel.plots.R ; \
echo outputPDF "<-" \"$CORE_PATH/$PROJECT/JOINT_CALL/$MDNA_HASH_ADDRESS/VQSR/variants.indel.plots.R.pdf\" ; \
awk 'NR > 25' $CORE_PATH/$PROJECT/JOINT_CALL/$MDNA_HASH_ADDRESS/VQSR/variants.indel.plots.R ) \
>| $CORE_PATH/$PROJECT/JOINT_CALL/$MDNA_HASH_ADDRESS/VQSR/variants.indel.plots.fixed.R

/isilon/sequencing/peng/softwares/R-3.1.1/bin/Rscript $CORE_PATH/$PROJECT/JOINT_CALL/$MDNA_HASH_ADDRESS/VQSR/variants.indel.plots.fixed.R
